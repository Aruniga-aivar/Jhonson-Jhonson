...
Pando Invoice Processing System
        ...
        logger.info(f":paperclip: Excel attachment added: {filename} ({len(excel_content)} bytes)")
        ...
        if attachment_size_mb > 25:  # 25MB is a common email limit
            logger.warning(f":warning:  Email attachment is large ({attachment_size_mb:.2f} MB) for {sender_email}. This might cause delivery issues.")
        
        logger.info("=" * 80)
        logger.info(f":incoming_envelope: INDIVIDUAL EMAIL MESSAGE CONSTRUCTED FOR: {sender_email}")
        logger.info("=" * 80)
        ...
        for attempt in range(1, retry_count + 1):
            try:
                ...
                with smtplib.SMTP(SMTP_SERVER, SMTP_PORT, timeout=30) as server:
                    ...
                    logger.info("=" * 80)
                    logger.info(f":rocket: SENDING EMAIL VIA SMTP TO: {sender_email}")
                    ...
                    if failed_recipients:
                        logger.error("=" * 80)
                        logger.error(f":x: EMAIL DELIVERY FAILED for {sender_email} on attempt {attempt}")
                    ...
                    logger.info("=" * 80)
                    logger.info(f":white_check_mark: :white_check_mark: :white_check_mark: EMAIL SUCCESSFULLY SENT TO {sender_email}! :white_check_mark: :white_check_mark: :white_check_mark:")
                    logger.info("=" * 80)
                    logger.info(f":e-mail: COMPLETE EMAIL DETAILS:")
                    ...
                    logger.info("=" * 80)
                    logger.info(f":email:  SMTP SERVER ({SMTP_SERVER}) ACCEPTED MESSAGE FOR DELIVERY")
                    logger.info(f":warning:  CRITICAL NOTE: Email was successfully sent by AWS Lambda")
                    logger.info(f":warning:  and accepted by Microsoft SMTP server for delivery.")
                    logger.info(f":warning:  If you don't receive it, it's being filtered by Gmail/Outlook")
                    logger.info(f":warning:  CHECK YOUR SPAM/JUNK FOLDER FOR:")
                    logger.info(f":warning:     Subject: {msg['Subject']}")
                    logger.info(f":warning:     From: {from_email}")
                    logger.info("=" * 80)
                    return True
